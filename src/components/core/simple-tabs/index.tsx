import { IonIcon, IonLabel, IonSegment, IonSegmentButton } from "@ionic/react";
import React from "react";

interface TabsProps {
  children: any[];
  value: number;
  onChange?;
  id?;
  style?;
}

interface TabProps {
  icon?;
  children;
  label;
}

export const Tab = (props: TabProps) => props.children;

export const Tabs = ({ children, value, onChange, id }: TabsProps) => (
  <div id={id} style={{ scrollMarginTop: "50px" }}>
    <IonSegment scrollable value={value.toString()}>
      {children.map((tab, index) => (
        <IonSegmentButton
          value={index.toString()}
          onClick={() => onChange(index)}
          key={index.toString()}
          style={{ maxWidth: "unset", textTransform: "unset" }}
        >
          {tab.props.icon && <IonIcon icon={tab.props.icon} />}
          <IonLabel className="ion-margin-top">{tab.props.label}</IonLabel>
        </IonSegmentButton>
      ))}
    </IonSegment>
    {children.map((tab, index) => (
      <div key={index} data-hidden={index !== value}>
        {tab}
      </div>
    ))}
  </div>
);
